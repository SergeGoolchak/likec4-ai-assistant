...
- `packages/core-pipeline` — `PipelineOrchestrator` и стадии 1-16 AI pipeline (…→ Proposal Generation → User Review → LikeC4 Generation → Validation → Architecture Review → Repair), с персистентностью после каждой стадии, естественной возобновляемостью после падения процесса, гейт-стадиями (`PipelineStageDef.isGate`) и внутренним repair-циклом (`repairStage`) с жёстким cap попыток и explicit stop при исчерпании. `assembleGeneratedFiles` — чистая функция, размещающая уже одобренный код предложений в гипотетическом слитом состоянии файлов (новые элементы — в отдельные файлы, модификации — точечная замена строк по `ArchitectureElement.sourceRef`).
