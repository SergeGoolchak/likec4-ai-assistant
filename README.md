...
- `packages/context-builder` — `ContextBuilder`: BFS-срез `ArchitectureGraph` по seed-элементам с trim по токен-бюджету + подмешивание knowledge-чанков; первый реальный потребитель — `proposal-generator` (Milestone 8).
...
- `packages/proposal-generator` — `LLMProposalGenerator`: превращает кандидатов на изменение в объяснимые `ProposalItem` (сборка контекста через `ContextBuilder`, честный учёт ответов на уточняющие вопросы — merge/reject-match/deferred), непустые `sources[]` и `Explanation` для каждого item; `repair()` точечно чинит один item по diagnostics/findings (Milestone 9).
- `packages/architecture-reviewer` — `RuleBasedArchitecturalReviewer`: детерминированный Level 2 rule-engine (обязательные metadata, naming conventions, лексические дубликаты с существующими элементами) — без LLM.
...
- `packages/core-pipeline` — `PipelineOrchestrator` и все 19 стадий AI pipeline (…→ Proposal Generation → User Review → LikeC4 Generation → Validation → Architecture Review → Repair → Diff → Preview → Apply), с персистентностью после каждой стадии, естественной возобновляемостью после падения процесса, гейт-стадиями (`PipelineStageDef.isGate`) и внутренним repair-циклом (`repairStage`) с жёстким cap попыток и explicit stop при исчерпании. `assembleGeneratedFiles` — чистая функция, размещающая уже одобренный код предложений в гипотетическом слитом состоянии файлов (новые элементы — в отдельные файлы, модификации — точечная замена строк по `ArchitectureElement.sourceRef`). `computeFileDiff` — сравнение existing/generated состояний файлов; `validation-gate.ts` — единая точка "Apply заблокирован?" (Level 1 + must-findings), переиспользуется repair/sessions/apply.
