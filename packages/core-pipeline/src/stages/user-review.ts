 ...
 * (approved/rejected/edited). 'pending' блокирует (item ещё не рассмотрен),
...
export const userReviewStage: PipelineStageDef = {
  ...
  isDone: (outputs) => (outputs.proposal?.items ?? []).every((item) => item.decision === 'approved' || item.decision === 'rejected' || item.decision === 'edited'),
  isGate: true,
  async run() {
